...
El objetivo de **Environmental Computing** es proporcionar tutoriales accesibles de codificación y estadística para usuarios de R. Si deseas obtener más información sobre este sitio, haz clic [aquí](https://environmentalcomputing.net/about-this-site/).

Este repositorio contiene el armazón de [blogdown](https://bookdown.org/yihui/blogdown/) y [Hugo](https://gohugo.io/) para construir y mantener el sitio web. A continuación se presentan detalles importantes para orientarte rápidamente en los entresijos de este sitio web.

### Instalación de `blogdown` y Hugo

`blogdown` es una interfaz de R para Hugo y es muy útil para nuestros propósitos, ya que la mayoría de los tutoriales están escritos en archivos R Markdown que se renderizan en un formato compatible con Hugo.

Blogdown se puede instalar desde CRAN o directamente desde el repositorio de GitHub de RStudio `blogdown` utilizando el paquete `remotes`:
